    ...
    for pico in picos_con_vino:
        if pico.vino.id not in lista_interna_vinos_id_vitte:
            ...
            for tamaño_copa in lista_tamaños:
                ...
                vino_in_db = crud.vino.create(db=db, obj_in=vino_in)
...
def sync_consumos_with_vitte_by_tarjeta(
    ...
    for consumo in consumos_vino:
        ...
        pedido, pudo_abirse, msg = crud.pedido.abrir_pedido(
            db=db,
            pedido_in=schemas.PedidoCreate(atendido_por=abierto_por_id),
        ...
        vino_in_db = crud.vino.get_by_vitte_name(db=db, vitte_name=consumo.vino, tamaño_copa=consumo.volumen)
        if not vino_in_db:
